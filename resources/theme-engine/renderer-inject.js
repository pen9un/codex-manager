((cssText, artDataUrl, wallpaperDataUrl, stampDataUrl, motionDataUrl, motionPosterDataUrl, themeConfig, initialVars, version) => {
  ...
  const detectShellMode = () => {
    ...
    // 本地适配：只识别原生完整标记，不把主题自身的 --dark 类当作用户偏好。
    const tokens = `${root.className || ""} ${body?.className || ""}`.toLowerCase().split(/\s+/);

    for (const element of [root, body]) {
      for (const name of ["data-theme", "data-appearance", "data-color-mode"]) {
        const value = (element?.getAttribute(name) || "").toLowerCase();
        if (["dark", "night"].includes(value)) return "dark";
        if (["light", "day"].includes(value)) return "light";
        if (["system", "auto"].includes(value)) return window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
      }
    }
    if (tokens.some(token => ["dark", "theme-dark", "appearance-dark"].includes(token))) return "dark";
    if (tokens.some(token => ["light", "theme-light", "appearance-light"].includes(token))) return "light";
    ...
    // 原创主题不从自己已经覆盖过的颜色反推原生模式。
    if (root.hasAttribute("data-original-theme")) return window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
